...
start_time = current_time.strftime("%Y-%m-%d %H:%M")  # Use current time as start time
end_time = (current_time + timedelta(days=1)).strftime("%Y-%m-%d %H:%M")  # Set end time to 1 day from start
...
for body, values in all_data.items():
    print(f"{body}: {values}")
